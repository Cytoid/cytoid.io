    ...
    <div class="my-2">
      <div class="card flex-shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
        <div v-if="status == ResetStatus.Nature" class="card-body">
          ...
          <div class="form-control">
            <div class="join w-full">
              ...
              <input id="reset-password" v-model="form.password" type="password" :placeholder="t('general.new_password_field_label')"
              :class="{
          ...
          <div class="form-control">
            <div class="join w-full">
              ...
              <input id="reset-password-again" v-model="form.passwordAgain" type="password" :placeholder="t('general.password_confirm_field_label')"
                :class="{
          ...
          <div class="form-control mt-2">
            <Captcha v-slot="{ verify }">
              <button :disabled="loading || !verified" class="btn btn-primary" @click="submit(verify)">{{ t('general.submit_btn') }}</button>
            </Captcha>
          </div>
        </div>
        ...
        <div v-else-if="status == ResetStatus.Success" class="card-body">
          ...
          <div class="form-control mt-2">
            <NuxtLink class="btn btn-success" to="/session/login">{{ t('general.login_btn') }}</NuxtLink>
          </div>
        </div>
        ...
        <div v-else class="card-body">
          ...
          <div class="form-control mt-2">
            <NuxtLink class="btn btn-secondary" to="/session/reset">{{ 'Try Again' }}</NuxtLink>
          </div>
        </div>
      </div>
    </div>
...
const submit = async (verify: () => Promise<string>) => {
  const password = form.value.password
